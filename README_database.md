...
- `script` (required): English name of the main script used by the orthography, e.g. Latin, Arabic, Armenian, Cyrillic, Greek, Hebrew. When a language uses a combination of several scripts in conjunction each script forms its own orthography. It should follow ISO 15924 (with a few exceptions documented in `lib/hyperglot/extra_data/script-names.yaml`).
=======
...
>>>>>>> 7e2c0a3 (Add currency/numerals/punctuation as Orthography object attributes via getters)
